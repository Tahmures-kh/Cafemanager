...
const filterTabs: Array<{ id: StaffFilter; label: string; description: string }> = [
    { id: "requested", label: "درخواست‌ها", description: "سفارش‌هایی که هنوز تحویل نشده‌اند" },
    { id: "delivered", label: "تحویل‌شده", description: "درخواست‌هایی که از انبار ارسال شده‌اند" },
];
...
function filterOrders(orders: CafeOrder[], filter: StaffFilter) {
    ...
    return (
        ...
        >
            <div className="flex items-start justify-between gap-3">
                <div>
                    <h3 className="text-sm font-black">درخواست #{order.id.slice(-6)}</h3>
...
export default function StaffDashboardPage() {
    ...
    const visibleOrders = useMemo(
        () => filterOrders(myCafeOrders, activeFilter),
        [myCafeOrders, activeFilter]
    );
    ...
    const requestedOrders = myCafeOrders.filter((order) => !isDeliveredStatus(order.status) && order.status !== "cancelled");
    const deliveredOrders = myCafeOrders.filter((order) => isDeliveredStatus(order.status));
    const pendingConfirmationOrders = myCafeOrders.filter((order) => order.status === "sent");
    ...
    return (
        <RoleGuard role="staff">
            ...
            <div className="mx-auto max-w-7xl p-5 lg:p-6">
                ...
                {confirmedMessage && (
                    <div className="mt-5">
                        <SuccessNotice message={confirmedMessage} onDismiss={() => setConfirmedMessage(null)} />
                    </div>
                )}

                <section className="mt-5 grid gap-3 md:grid-cols-2">
                    <div className="penza-card rounded-[1.25rem] p-4">
                        <p className="text-sm font-black text-slate-500">درخواست کالا</p>
                        <p className="mt-2 text-3xl font-black text-[#0B2F0B]">{formatNumber(requestedOrders.length)}</p>
                        <p className="mt-1 text-xs font-bold leading-6 text-slate-500">هنوز تحویل نشده‌اند</p>
                    </div>
                    <div className="penza-card rounded-[1.25rem] p-4">
                        <p className="text-sm font-black text-slate-500">تحویل درخواست</p>
                        <p className="mt-2 text-3xl font-black text-[#0B2F0B]">{formatNumber(deliveredOrders.length)}</p>
                        <p className="mt-1 text-xs font-bold leading-6 text-slate-500">از طرف انبار ارسال شده‌اند</p>
                    </div>
                </section>
                ...
                <section className="mt-5 grid gap-5 xl:grid-cols-[23rem_1fr]">
                    <aside className="space-y-4">
                        <div className="penza-card rounded-[1.5rem] p-4">
                            <h2 className="text-lg font-black text-[#0B2F0B]">نمای سفارش‌ها</h2>
                                    ...
                                    <button
                                        ...
                                        className={
                                            activeFilter === filter.id
                                                ? "relative rounded-2xl bg-[#00A300] px-4 py-3 text-right text-sm font-black text-white shadow-sm"
                                                : "relative rounded-2xl border border-green-900/10 bg-white px-4 py-3 text-right text-sm font-black text-slate-600 hover:bg-[#f2fff2]"
                                        }
                                    >
                                        <span className="flex items-center gap-2">
                                            {filter.label}
                                            {filter.id === "delivered" && pendingConfirmationOrders.length > 0 && (
                                                <span className="rounded-full bg-[#c99a00] px-2 py-0.5 text-[11px] font-black text-white">
                                                    {formatNumber(pendingConfirmationOrders.length)} منتظر تایید
                                                </span>
                                            )}
                                        </span>
                                        <span className={activeFilter === filter.id ? "mt-1 block text-xs font-bold text-white/70" : "mt-1 block text-xs font-bold text-slate-400"}>{filter.description}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                                                ...
                                                return (
                                                    <tr key={item.id} className="hover:bg-[#f8fff8]">
                                                        ...
                                                        <td className="px-4 py-3 font-bold text-slate-500">
                                                            {remaining > 0 ? `مانده ${formatOrderQuantity(product, remaining)}` : "کامل"}
                                                        </td>
                                                    </tr>
                                                );
                                            })}
