...
**Current version:** V1.22 completed, V1.23 handoff Bible  
...
## V1.23 Update — Dual Counting Units

Implemented the first version of separate counting units:

```text
Stock/base unit = real inventory unit used for stock and recipe costing
Ordering/formal unit = how cafe staff requests the item
Conversion = how much stock is consumed by 1 ordering unit
```
...
- stock unit: بطری
- ordering unit: شلف
- conversion: 1 شلف = 6 بطری
- ordering step: whole شلف only

پنیر موتزارلا:
- stock unit: کیلوگرم
- ordering unit: بسته
- conversion: 1 بسته = 2 کیلوگرم
- ordering step: whole بسته only
```

Important behavior:

```text
Cafe staff enters request quantity in ordering unit.
Storage stock list shows real stock in stock/base unit.
Storage order packing is still entered in ordering unit.
When request is delivered, inventory is deducted in stock/base unit.
```

Example:

```text
Cafe requests 1 شلف شیر بطری
Storage inventory decreases by 6 بطری

Cafe requests 1 بسته پنیر موتزارلا
Storage inventory decreases by 2 کیلوگرم
```

A new helper file exists:

```text
lib/product-units.ts
```
...
Store key changed to:

```text
cafe-storage-mvp-store-v4-dual-units
```

This intentionally resets old browser demo data so the updated milk/mozzarella conversion fields appear correctly.

---

## 12.5. V1.24 Update — Staff Favorites + Manager Daily Cards

Implemented after V1.23:

```text
Staff can star/unstar any product on /staff/request to mark it as a favorite ✅
New "★ علاقه‌مندی‌ها" filter chip shows only favorited products ✅
"★ کالاهای پرتکرار" quick-reorder shelf appears above the product grid when
favorites exist, with one-tap +1 add-to-cart buttons ✅
Favorites are stored in their own localStorage key
(cafe-storage-mvp-favorites-v1), independent of the main order/inventory
store, so resetting test data does not clear favorites ✅
New useFavoriteProducts() hook added to lib/local-store.ts
(favoriteIds, isFavorite, toggleFavorite) ✅
Manager dashboard now shows an "امروز" (today) row of 4 daily cards above
the existing overview cards: درخواست‌های امروز, تحویل‌های امروز,
گردش کالای امروز, کالای کم‌موجود ✅
Daily cards are computed client-side from existing store data
(orders.createdAt / updatedAt, stockMovements.createdAt, inventory
minimumQuantity) — no new store fields required ✅
```

Notes:

```text
Favorites are shared per browser (no per-user account system yet), matching
the current single-role-per-browser MVP model.
Daily cards use plain calendar-day comparison in the browser's local time
zone (no server, no timezone config yet).
Workflow language unchanged: still only درخواست کالا and تحویل درخواست.
No urgent/critical/problem UI was reintroduced.
```
...
Dual stock/order counting units added ✅
Milk shell conversion added ✅
Mozzarella package-to-kg conversion added ✅
Real database missing ❌
Real authentication missing ❌
Native .xlsx export missing ❌
Staff favorites / quick-reorder shelf added ✅
Manager daily summary cards added ✅
```
